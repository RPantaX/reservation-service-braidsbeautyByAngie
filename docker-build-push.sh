...
echo "🐳 Docker Build & Push Script para reservation Service"
echo "=================================================="
echo ""
...
# Verificar JAR
JAR_PATH="application/target/application-0.0.1-SNAPSHOT.jar"
if [ -f "$JAR_PATH" ]; then
    JAR_SIZE=$(du -h "$JAR_PATH" | cut -f1)
    print_success "JAR encontrado: $JAR_PATH ($JAR_SIZE)"
else
    print_error "JAR no encontrado en $JAR_PATH"
    print_info "Archivos en application/target/:"
...
CONTAINER_ID=$(docker run -d -p 8083:8082 --name reservation-service-test "$VERSIONED_TAG")
...
if [ $? -eq 0 ]; then
    ...
    if docker ps | grep -q "reservation-service-test"; then
        print_success "Contenedor ejecutándose correctamente"

        # Intentar hacer health check
        print_info "Verificando health check..."
        if docker exec reservation-service-test curl -f http://localhost:8082/actuator/health 2>/dev/null; then
        ...
        docker logs --tail 10 reservation-service-test

    else
        print_error "El contenedor se detuvo inesperadamente"
        print_info "Logs del contenedor:"
        docker logs reservation-service-test
    fi

    # Limpiar contenedor de prueba
    print_info "Limpiando contenedor de prueba..."
    docker stop reservation-service-test >/dev/null 2>&1 || true
    docker rm reservation-service-test >/dev/null 2>&1 || true
